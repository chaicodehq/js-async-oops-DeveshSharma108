...
export function checkIngredients(ingredient) {
  const ingredients = new Set(["tea", "milk", "sugar", "ginger", "cardamom"]);
  return new Promise((resolve, reject) => {
...
export function prepareChaiWithTimeout(type, timeoutMs) {
  function rejectionAfterTimeOut(timeoutMs) {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
  ...
  return Promise.race([orderChai(type, 1), rejectionAfterTimeOut(timeoutMs)]);
}
...
export function processChaiQueue(orders) {
  orders = orders.map((order) => {
    return new Promise(async (resolve, reject) => {
      try {
        const value = await orderChai(order.type, order.quantity);
        resolve(value);
      } catch (e) {
        reject(e.message);
      }
